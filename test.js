...
app.post('/dest/:desti', function(req, res){
    ...
    ref.once('value',function(snapshot){
        ...
        if( childData.BUSSTOP_NAME == dest ){
          ...
          request($api_url, function(rq_err,rq_res,rq_data){
            ...
            for( var i = 0; i < obj.BUSSTOP_LIST.length; i++){
              ...
              Answer.push( '버"\n"스이름 : ' +obj.BUSSTOP_LIST[i].LINE_NAME +"\n")
              Answer.push('방\n향 : ' + childData.NEXT_STATION+' 방향' + "\n")
              Answer.push('남은시간 : ' +  obj.BUSSTOP_LIST[i].REMAIN_MIN + '분' + "\n")
              Answer.push('남은 정류장 수 : ' + obj.BUSSTOP_LIST[i].REMAIN_STOP +'개'+ "\n");
      ...
      //setTimeout(function(){   Answer = JSON.parse(Answer)  } , 1200);
      setTimeout(function(){ res.send( { message : Answer } )   } , 1300);
});
